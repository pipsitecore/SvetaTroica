...
function calculateTax() {
	...
	//validate input
	if (isempty === "" || floor === "" || people === "" || dog === "") {
		alert("Пропуснали сте поле");
		return;
	}
	//Check to see if apartment is empty and fix the total to 6.5 lev
	if (isempty === "0") {
		var total = 6.5
	} else {
	//Else calculate the tax
		var total = parseFloat(people) + parseFloat(dog) + parseFloat(floor)
	}
	//Display the tax
